...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	meanSpeedRun := meanSpeed(action, duration)
	calories = (runningCaloriesMeanSpeedMultiplier * meanSpeedRun * runningCaloriesMeanSpeedShift) *
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	kvadrMeanSpeedInMsec := math.Pow(meanSpeedInMsec, 2)
	heightInMetr := height / cmInM
	calories = ((walkingCaloriesWeightMultiplier*weight + (kvadrMeanSpeedInMsec/
		heightInMetr)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	var calories float64
	meanSpeedInPool := swimmingMeanSpeed(lengthPool, countPool, duration)
	calories = (meanSpeedInPool + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier *
		weight * duration
	return calories
}
